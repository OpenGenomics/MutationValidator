    ...
    for pileupcolumn in bam_file.pileup(chrom, position-1, position):
        if pileupcolumn.pos == position-1:
            ...
            for pileupread in pileupcolumn.pileups:
                if pileupread.is_del == 1 | pileupread.indel:
                    #print('im here')
                    indel_counts += 1
                ...
                if  pileupread.query_position is None:
                    print "pileup.alignment.query_position is None: skip this alignment "
                    continue
...
def validate_indel_mut_bam_position(sample,type_bam,bam,bam_number,position_list,output_file,rtype):
...
if len(positions)==0:
    ...
    for fl in files:
        ...
        validate_indel_mut_bam_position(os.path.basename(fl).strip().split(".")[0],detailed_type,fl,bam_number,positions,results, args.rnatype)
